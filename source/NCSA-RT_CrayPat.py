...
#                    line 1: filename1, legend_name1, Pat_Region_name1[optional], nThread[optional]
#                       .....
#                    line n: filename(n), legend_name(n), Pat_Region_name(n)[optional],  nThread[optional]
#              - Using the second command line argument, Pat_Region name can be set. (Default: None)
...
if A[0].split()[0] == 'CrayPat/X:':
   ...
   islist = True
   # Read a Pat_Region name
   try:
      PatRegion_Name = sys.argv[2]
   except IndexError:
      print ("   [Warning] You can specify a Pat_Region name for plots using the second command line argument. Now it is set to 'None'. ")
      PatRegion_Name = 'None'
   ...
   for line in A:
      ...
      try:
         ...
         reports.append([ Rep,line.split()[1],PatRegion_Name ] )     # Report, legend_name, Region_name
   ...
   if PatRegion_Name != 'None':
      ...
      # Plot filename
      plot_fname = fname+'_'+''.join(e for e in PatRegion_Name if e.isalnum())
